...
class FacePipeline:
    ...
    def detect_faces(self, frame, model_name='SSD DNN', conf_threshold=0.5):
        ...
        if model_name == 'Haar Cascade':
            ...
            detections = self.detector_cascade.detectMultiScale(gray, 1.1, 5, minSize=(30, 30))
            for (x, y, width, height) in detections:
                results.append({'box': (int(x), int(y), int(width), int(height)), 'landmarks': [], 'confidence': 1.0})
            ...
            for i in range(detections.shape[2]):
                ...
                if confidence >= conf_threshold:
                    ...
                    x2 = min(w - 1, x2)
                    y2 = min(h - 1, y2)
                    results.append({'box': (x1, y1, x2 - x1, y2 - y1), 'landmarks': [], 'confidence': confidence})
            ...
            if retval and faces is not None:
                for f in faces:
                    ...
                    if confidence >= conf_threshold:
                        ...
                        results.append({'box': (x, y, width, height), 'landmarks': landmarks, 'confidence': confidence, 'raw_face_data': f})
        return results
    ...
    def process_and_recognize(self, frame, detector_name='SSD DNN', recognizer_name='SFace DNN', conf_threshold=0.5, similarity_threshold=0.4):
        ...
        for det in detections:
            ...
            results.append({'box': (x, y, w, h), 'landmarks': landmarks, 'name': final_name, 'similarity': best_similarity, 'confidence': confidence})
        return results
